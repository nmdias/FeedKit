// swift-tools-version: 5.9
// The swift-tools-version declares the minimum version of Swift required to build this package.

import PackageDescription

let package = Package(
  name: "FeedKit",
  platforms: [
    .macOS(.v12),
    .iOS(.v15),
    .watchOS(.v8),
    .tvOS(.v15),
    .visionOS(.v1)
  ],
  products: [
    .library(
      name: "FeedKit",
      targets: [
        "FeedKit"
      ]
    ),
    .library(
      name: "XMLKit",
      targets: [
        "XMLKit"
      ]
    )
  ],
  targets: [
    .target(
      name: "XMLKit"
    ),
    .testTarget(
      name: "XMLKitTests",
      dependencies: ["XMLKit"],
      resources: [
        .process("Resources/xml/Sample.xml")
      ]
    ),
    .target(
      name: "FeedKit",
      dependencies: [
        "XMLKit"
      ]
    ),
    .testTarget(
      name: "FeedKitTests",
      dependencies: [
        "FeedKit"
      ],
      resources: [
        .process("Resources/json/feed.json"),
        .process("Resources/json/feed_v1_1.json"),
        .process("Resources/xml/Ampersand.xml"),
        .process("Resources/xml/Atom + XHTML.xml"),
        .process("Resources/xml/Atom.xml"),
        .process("Resources/xml/AtomLongProlog.xml"),
        .process("Resources/xml/AtomMedia.xml"),
        .process("Resources/xml/AtomFeedHistory.xml"),
        .process("Resources/xml/AtomCommentAPI.xml"),
        .process("Resources/xml/AtomPodloveSimpleChapters.xml"),
        .process("Resources/xml/Content.xml"),
        .process("Resources/xml/FeedNotFound.xml"),
        .process("Resources/xml/RSS.xml"),
        .process("Resources/xml/RSSDC.xml"),
        .process("Resources/xml/RSSFeedHistory.xml"),
        .process("Resources/xml/RDF.xml"),
        .process("Resources/xml/RDFDC.xml"),
        .process("Resources/xml/CommentAPI.xml"),
        .process("Resources/xml/PodloveSimpleChapters.xml"),
        .process("Resources/xml/RSSAtom.xml"),
        .process("Resources/xml/Media.xml"),
        .process("Resources/xml/NetNewsWire.xml"),
        .process("Resources/xml/Syndication.xml"),
        .process("Resources/xml/iTunes.xml"),
        .process("Resources/xml/YouTube.xml"),
        .process("Resources/xml/GeoRSSSimple.xml"),
        .process("Resources/xml/Podcast.xml")
      ]
    )
  ]
)
